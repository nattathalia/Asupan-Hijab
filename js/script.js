const videos = [ 

{
  title:"Bokep bocil kampung",
  embed:
  "https://luluvdo.com/e/erw24ughxr6i",
},
{
   title:"Video Bokep Cewe Perawan Jilbab Biru",
   embed:
   "https://luluvdo.com/e/k474yt3o7rhb",
},
{
   title:"Video Bokep Cewe Perawan Jilbab Biru",
   embed:"https://luluvdo.com/e/1gdfzrousrji",
},
{
  title:"Toket Bulat Hijabers Isna",
  embed:
  "https://luluvdo.com/e/ys8enm21ko0n",
},
{
   title:"Bokep Sex Terbaru Perawan Hijab Streaming",
   embed:
  "https://luluvdo.com/e/xwgk5z02eaxy",
},
{
   title:"ABG Berjilbab Mesum Sama Pacar Terbaru",
   embed:
   "https://luluvdo.com/e/k53gqyutwbfa",
},
{
  title:"Saatnya oral seks sambil berhijab",
  embed:
  "https://luluvdo.com/e/4v8pettn0j2w"
},
{
  title:"Remaja Muslim berhijab disetubuhi dengan kasar dan dikucuri sperma",
  embed:"https://luluvdo.com/e/idyyrakr5bgr"
},
{
  title:"Putri berhijab belajar cara mengisap kontol",
  embed:"https://luluvdo.com/e/1ungrpereb8j"
},
{
  title:"Ibu tiri berhijab belajar cara melayani pria dari anak tirinya",
  embed:"https://luluvdo.com/e/wkrefd3sl5wz"
},
{
  title:"Gadis Muslim disemprot sperma",
  embed:"https://luluvdo.com/e/pqt6vfbtoz8e"
},
{
  title:"Xvideos shake it first before putting it in SD",
  embed:
  "https://luluvdo.com/e/6mtdn8yfookj"
},
{
  title: "Xvideos hijab teen has a naughty proposal for her coach",
  embed: 
  "https://luluvdo.com/e/ejo2juvwelha"
},
{
  title: "Xvideos hijab teen alexia anders is excited to finally have her",
  embed:
  "https://luluvdo.com/e/1vbeuvw00xrg"
},
{
  title: "Xvideos arab sex hijab stepsister saudi fucks deliciously",
  embed:
  "https://luluvdo.com/e/yzy5prwrosin"
},
{
  title: "Anal hijab tobrut pertama kali kesakitan",
  embed: 
  "https://luluvdo.com/e/t5no683qlkiq"
},
{
  title: "G0yangan Pacar Hijab Cantik Mmxnya Mntap",
  embed: 
  "https://luluvdo.com/e/d33s8ouspsf0"
},
{
  title: "tanye hijab pap colmek ke selingkuhannya",
  embed:
  "https://luluvdo.com/e/l0rqexugyrji"
},
{
  title: "Hijab umay bikin konten ekslusif",
  embed: 
  "https://luluvdo.com/e/23b3j0hae8d9"
},
{
  title: "Bokep indo - Hijab Cantik Sange Malu Malu",
  embed: 
  "https://luluvdo.com/e/3n94zibmjvqu"
},
{
  title: "Abg Hijab Baru Pertama Kali Nyobain",
  embed: 
  "https://luluvdo.com/e/gwdpb400irja"
},
{
  title: "133737 mp4",
  embed: 
  "https://luluvdo.com/e/5p02q9p55jzw"
},
{
  title: "Bokep Indo Hijab Kacamata SMA Batik Colmek",
  embed:
  "https://luluvdo.com/e/72dazlfqaoyz"
},
{
  title: "Sex ABG Jilbab Indonesia Masturbasi",
  embed: 
  "https://luluvdo.com/e/y6mfgjkhzxsk"
},
{
  title: "Ngentot Cewe Jilbab SMA",
  embed: 
  "https://luluvdo.com/e/86yk4l0mhvhd"
},
{
  title: "Jilbab Tocil Memek Mulus",
  embed: 
  "https://luluvdo.com/e/0mamr14ilt2y"
},
{
  title: "Jilbab Tobrut Ngentot Sampe Becek",
  embed:
  "https://luluvdo.com/e/4y5scyalmv1z"
},
{
  title: "Jilbab Semok",
  embed:
  "https://luluvdo.com/e/w3rw7z1f4zxl"
},
{
  title: "Jilbab SMA Ngewe Bikin Konten",
  embed: 
  "https://luluvdo.com/e/25skfdiklmfs"
},
{
  title: "Jilbab Kepengen",
  embed: 
  "https://luluvdo.com/e/ly7z4k9huspg"
},
{
  title: "Ceweknya Cakep",
  embed: 
  "https://luluvdo.com/e/pbol01fxw3uf"
},
{
  title: "ABG Jilbab Masturbasi",
  embed: 
  "https://luluvdo.com/e/d32l7mzcvqcg"
},
{
  title: "ABG Hijab Cakep Buka Jilbab Benerin Tetek",
  embed: 
  "https://luluvdo.com/e/jurphqq5sq5k"
},
{
  title: "Skandal Hijab Mirip Influencer Terkenal",
  embed: 
  "https://luluvdo.com/e/gxcsshn1yvpv"
},
{
  title: "Pacar Hijab Menikmati Banget",
  embed: 
  "https://luluvdo.com/e/7saveth4cxrm"
},
{
  title: "Mahasiswi Jilbab Sange dari Bandung",
  embed: 
  "https://luluvdo.com/e/x4li42iqpnfz"
},
{
  title: "Mahasiswi Jilbab Colmek Di Toilet Kampus",
  embed: 
  "https://luluvdo.com/e/vyy8g41o78m9"
},
{
  title: "Hijabers Della Ajarin Pacar Belum Pernah Ngewe",
  embed: 
  "https://luluvdo.com/e/ezu4kewi1ygy"
},
{
  title: "Hijaber Eks Pacar Katanya Belum Pernah Dipegang Lagi",
  embed:
  "https://luluvdo.com/e/hrryhgl22c82"
},
{
  title: "Hijab Kebaya Ungu Meki Gundul",
  embed: 
  "https://luluvdo.com/e/gqoksxhaubfm"
},
{
  title: "Gen-Z Main Diparkiran Malah Tambah Nafsu",
  embed:
  "https://luluvdo.com/e/pa5ky1y6qgtt"
},
{
  title: "Enak Kalo Punya Pacar Tante Hijab Tiap Hari Dimanjain",
  embed:
  "https://luluvdo.com/e/lcghvu7n6v2e"
},
];

const container = document.getElementById("videoContainer");

videos.forEach(video => {
  const card = document.createElement("div");
  card.className = "video-card";

  card.innerHTML = `
    <h3>${video.title}</h3>
    <iframe
      src="${video.embed}"
      width="100%"
      height="360"
      frameborder="0"
      scrolling="no"
      allowfullscreen
      webkitallowfullscreen
      mozallowfullscreen>
    </iframe>
  `;

  container.appendChild(card);
});

// ==========================
// SIDEBAR
// ==========================

// Tempel kode sidebar kamu di sini


// ==========================
// SEARCH
// ==========================

// Tempel kode pencarian kamu di sini


// ==========================
// PAGINATION
// ==========================

// Tempel kode pagination kamu di sini


// ==========================
// RENDER VIDEO
// ==========================

const page = document.body.dataset.page || "home";
const container = document.getElementById("videoContainer");

if (container && videos[page]) {
  videos[page].forEach(video => {
    const card = document.createElement("div");
    card.className = "video-card";

    card.innerHTML = `
      <iframe src="${video.embed}" allowfullscreen></iframe>
      <div class="video-info">
        <div class="video-title">${video.title}</div>
      </div>
    `;

    container.appendChild(card);
  });
}
